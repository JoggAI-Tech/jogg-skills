...
class DesignSystemGenerator:
    ...
    def _apply_reasoning(self, category: str, search_results: dict) -> dict:
        ...
        if not rule:
            raise ValueError(f"reasoning search returned no matching rule for category: {category}")
        ...
        try:
            ...
            raise ValueError(f"reasoning rule contains invalid Decision_Rules JSON: {category}") from exc
    ...
    def _require_search_results(self, domain: str, search_result: dict) -> list:
        """Return genuine ranked hits or stop design generation."""
        if not isinstance(search_result, dict):
            raise ValueError(f"{domain} search returned an invalid response")
        if search_result.get("error"):
            raise ValueError(f"{domain} search failed: {search_result['error']}")
        results = search_result.get("results")
        if not isinstance(results, list) or not results:
            raise ValueError(f"{domain} search returned no matching result")
        return results

    def _require_field(self, domain: str, result: dict, field: str) -> str:
        value = result.get(field)
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"{domain} search result is missing required field: {field}")
        return value.strip()
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        product_results = self._require_search_results("product", product_result)
        category = self._require_field("product", product_results[0], "Product Type")
        ...
        style_results = self._require_search_results("style", search_results.get("style", {}))
        color_results = self._require_search_results("color", search_results.get("color", {}))
        typography_results = self._require_search_results(
            "typography", search_results.get("typography", {})
        )
        landing_results = self._require_search_results("landing", search_results.get("landing", {}))
        ...
        self._require_field("style", best_style, "Style Category")
        for field in ("Primary", "Accent", "Background", "Foreground", "Destructive"):
            self._require_field("color", best_color, field)
        for field in ("Pattern Name", "Section Order"):
            self._require_field("landing", best_landing, field)
        for field in ("Heading Font", "Body Font"):
            self._require_field("typography", best_typography, field)
        ...
        return {
            ...
            "pattern": {
                ...
                "sections": best_landing.get("Section Order", ""),
                "cta_placement": best_landing.get("Primary CTA Placement", ""),
                "color_strategy": best_landing.get("Color Strategy", ""),
                "conversion": best_landing.get("Conversion Optimization", "")
            },
            "style": {
                "name": best_style.get("Style Category", ""),
            ...
            "colors": {
                ...
                "cta": best_color.get("Accent", ""),
                "text": best_color.get("Foreground", ""),
            },
            "typography": {
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("### Typography")
    lines.append("")
    lines.append(f"- **Heading Font:** {typography['heading']}")
    lines.append(f"- **Body Font:** {typography['body']}")
    if typography.get("mood"):
        lines.append(f"- **Mood:** {typography.get('mood', '')}")
    if typography.get("google_fonts_url"):
    ...
    lines.append(f"  background: {colors['cta']};")
    ...
    lines.append(f"  border: 2px solid {colors['primary']};")
    lines.append("  padding: 12px 24px;")
    lines.append("  border-radius: 8px;")
    lines.append("  font-weight: 600;")
    lines.append("  transition: all 200ms ease;")
    ...
    lines.append(f"  border-color: {colors['primary']};")
    lines.append("  outline: none;")
    lines.append(f"  box-shadow: 0 0 0 3px {colors['primary']}20;")
    lines.append("}")
    lines.append("```")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append(f"> **PROJECT:** {project}")
    lines.append(f"> **Generated:** {timestamp}")
    lines.append(f"> **Page Type:** {page_overrides['page_type']}")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    # Page overrides are optional, but when requested they must also be evidence-backed.
    domain_results = {}
    for domain, result in (
        ...
        if not isinstance(result, dict):
            raise ValueError(f"{domain} page search returned an invalid response")
        if result.get("error"):
            raise ValueError(f"{domain} page search failed: {result['error']}")
        matches = result.get("results")
        if not isinstance(matches, list) or not matches:
            raise ValueError(f"{domain} page search returned no matching result")
        domain_results[domain] = matches

    style_results = domain_results["style"]
    ux_results = domain_results["ux"]
    landing_results = domain_results["landing"]
    ...
    if not layout:
        raise ValueError("page searches returned no supported layout evidence")

    if not recommendations:
        raise ValueError("page searches returned no supported recommendation evidence")
...
def _detect_page_type(context: str, style_results: list) -> str:
    ...
    # Infer from the matched style evidence when the query has no explicit page type.
    if style_results:
    ...
    raise ValueError("page type cannot be derived from the requested context or style evidence")
