...
Use the complete Brief, complete script, and every complete Slide intent. The
builder binds all of them into the immutable input hash and preserves unknown
source-bound fields. Product classification uses only the explicit private
`design_domain`; it never asks the search engine to guess a product category from
the narration. Style refinement uses the confirmed visual tone and Slide
relationships. Do not build one MASTER per shot.
...
priority, reading order, simplification, and final meaning. The Slide designer
combines both authorities. It does not select a template or copy a source layout.
...
confirmed Brief, complete script, Slide set, or whole-video visual direction
changes.
