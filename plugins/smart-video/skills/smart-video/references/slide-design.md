...
- Give each Slide one primary conclusion that a viewer can understand within one
  second at the stable final frame.
- Use no more than five primary visual objects. Treat a coherent group that
  communicates one relationship as one object; do not split narration facts into
  separate shapes merely to fill the frame.
- Use no more than three supporting labels beyond source-required copy.
- Prefer scale, position, alignment, whitespace, and connection before assigning a
  new container or color.
- Add a container only when it expresses grouping, comparison, or a boundary. Do
  not mechanically turn semantic objects into cards, boxes, or steps.
- Before composing, choose a closed color-role budget of no more than five
  semantic color roles from the MASTER tokens. Use only that set for the entire
  Slide; repeated use of one role does not create a new role.
- Let motion reveal reading order or change; it must not introduce extra visual
  objects or decorative events.
- Simplification must preserve every required fact, qualifier, unit, uncertainty,
  and attribution. If those facts cannot remain legible within these limits, stop
  and report the conflict instead of deleting meaning.
