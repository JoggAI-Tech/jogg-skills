...
Read [planning-contracts.md](references/planning-contracts.md) and [mg-director-visual-contract.md](references/mg-director-visual-contract.md) before authoring a plan. Read [html-mg-contract.md](references/html-mg-contract.md) only when implementing the director-selected HTML/MG. These contracts are extracted from Podcastor's Video Studio planner at the source commit recorded in `extraction-manifest.json`.
